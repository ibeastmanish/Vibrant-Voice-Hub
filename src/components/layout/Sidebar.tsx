    ...
    <div className="w-64 h-screen bg-black/40 backdrop-blur-xl border-r border-white/10 p-6 flex flex-col justify-between transition-colors duration-500 z-10 shrink-0">
      <div>
        <div className="flex items-center gap-3 mb-12">
          <div className={cn("w-8 h-8 rounded-full bg-current opacity-20", brandColors[activeBrand])} />
          <h1 className="text-xl font-bold tracking-tight">Vibrant Voice Hub</h1>
        </div>
        ...
        <nav className="space-y-2">
          {navItems.map((item) => (
            ...
            >
              <span className={activeView === item.view ? brandColors[activeBrand] : ""}>
                {item.icon}
              </span>
      ...
      <div className="flex items-center gap-3 px-4 py-3 bg-white/5 rounded-xl border border-white/5">
        ...
        <span className="text-sm font-medium text-white/70">System Node: Ready</span>
      </div>
    </div>
  );
};
