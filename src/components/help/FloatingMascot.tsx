        ...
        {voiceState === 'error' && (
          ...
          >
            <div className="bg-white/10 backdrop-blur-xl border border-white/20 rounded-2xl p-4 shadow-xl max-w-xs text-right">
              <p className="text-sm font-medium text-white/90">Didn't quite catch that—let's try that one again!</p>
            </div>
            ...
            <div className="flex gap-2 mr-2 overflow-x-auto max-w-[300px] pb-2 hide-scrollbar">
              {fallbacks.map((item, i) => (
                <button key={i} className="flex items-center gap-2 px-4 py-2 bg-black/50 border border-white/10 rounded-full text-xs font-medium text-white/80 hover:bg-white/10 hover:text-white transition-colors whitespace-nowrap">
                  {item.icon}
                  {item.label}
                </button>
              ))}
